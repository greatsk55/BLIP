import { reportComment } from '@/lib/board/actions';
...
export async function POST(request: Request) {
  try {
    ...
    const rateCheck = await checkRateLimit(`api:comment:report:${ip}`, REPORT_LIMIT);
    ...
    const { boardId, commentId, authKeyHash, reason } = body;
    if (!isString(boardId) || !isString(commentId) || !isString(authKeyHash) || !isString(reason)) {
    ...
    const result = await reportComment(boardId, commentId, authKeyHash, reason as 'spam' | 'abuse' | 'illegal' | 'other');
